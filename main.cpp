...
public:
	...
	void saveRenderedImage() {
		...
		cv::Mat output_mat(HEIGHT, WIDTH, CV_8UC4);

		for (uint32_t image_h = 0; image_h < HEIGHT; image_h++) {
			for (uint32_t image_w = 0; image_w < WIDTH; image_w++) {
				uint32_t image_index = image_h*WIDTH + image_w;
				output_mat.at<cv::Vec4b>(image_h, image_w)[0] = 255.0f*pmappedMemory[image_index].r;
				output_mat.at<cv::Vec4b>(image_h, image_w)[1] = 255.0f*pmappedMemory[image_index].g;
				output_mat.at<cv::Vec4b>(image_h, image_w)[2] = 255.0f*pmappedMemory[image_index].b;
				output_mat.at<cv::Vec4b>(image_h, image_w)[3] = 255.0f*pmappedMemory[image_index].a;
			}
		}
		...
		//cv::imshow("Result", output_mat);
		//cv::waitKey(0);
		cv::imwrite("result.png", output_mat);
	...
	void createComputePipeline() {
		...
		*/
		...
		const char* shader_path = "D://visuaL_Project//first_project//VulkanTest//ComputeShader//comute_shader.comp.spv";
		uint32_t* code = readFile(filelength, shader_path);
