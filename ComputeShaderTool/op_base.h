...
namespace ImageProcess {
	...
	public:
		...
		virtual void ComputeConvOutputShape(PaddingMode padding_mode, int& padding_top, int& padding_left, const int& in_h, const int& in_w,
			...
			int& out_h,int& out_w, int dilation_h = 1,int dilation_w = 1, int stride_h = 1,  int stride_w = 1);
		virtual bool Forward(std::vector<Tensor>& input_tensor,
			std::vector<Tensor>& blobs,
			std::vector<Tensor>& out_tensor) = 0;
		/*virtual bool Forward(std::vector<std::shared_ptr<VulkanUtil::VulkanImage>>& input_image,
			std::vector<std::shared_ptr<VulkanUtil::VulkanImage>>& blobs_image,
			std::vector<std::shared_ptr<VulkanUtil::VulkanImage>>& out_image) = 0;*/

		std::shared_ptr<VulkanUtil::VulkanDevice>& GetVulkanDevice() {
			return my_device_;
		}
